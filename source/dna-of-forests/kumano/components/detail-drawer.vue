      ...
      section.memo(v-if="($root.$i18n.locale === 'en' && memofig_width) || ($root.$i18n.locale === 'ja' && memo)")
        ...
        .bg_line
          dl(v-if="$root.$i18n.locale === 'ja'" v-for="(answer, question) in memo")
            dt {{ question }}
            dd(v-html="answer")
          img(:alt="$t('detail_drawer.sample.article.memo.sketch')" v-if="memofig_width" v-bind:src="'/dna-of-forests/niho/img/detail-drawer/memo/'+id+'.png'" v-bind:style="{ width: memofig_width }")
...
.image_wrapper
  ...
  &:before
    ...
    left: calc(50% - 16px)
    z-index: 1
    background: transparent url(/dna-of-forests/niho/img/detail-drawer/loader.png) no-repeat center
...
.bg_line
  background-image: url(/dna-of-forests/niho/img/detail-drawer/memo-line.png)
  padding-bottom: 1px
  text-align: center
