...
#container(:data-lang="$root.$i18n.locale")
  ...
  .marker.sample(v-for="(item, index) in samples" v-bind:id="'s-'+(index+1)" v-bind:class="{ selected: $route.path=='/'+$route.params.forest+'/panorama/s-'+(index+1) }" v-on:click="goMarker('s-'+(index+1))")
  ...
  .marker.knowledge(v-for="(item, index) in knowledges" v-bind:id="'k-'+(index+1)" v-bind:class="{ selected: $route.path=='/'+$route.params.forest+'/panorama/k-'+(index+1) }" v-on:click="goMarker('k-'+(index+1))")
    //- 日本語なら画像、英語ならテキストでラベルを表示
